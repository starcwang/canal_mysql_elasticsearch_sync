...
import com.star.sync.elasticsearch.config.MappingConfig;
...
public class InsertCanalListener extends AbstractCanalListener<InsertCanalEvent> {
    ...
    protected void doSync(String database, String table, String index, String type, RowData rowData) {
        ...
        String primaryKey = Optional.ofNullable(mappingConfig.getTablePrimaryKeyMap().get(database + "." + table)).orElse("id");
        ...
        String json = JsonUtil.toJson(parseColumnsToMap(columns));
        elasticsearchService.insertById(index, type, idColumn.getValue(), json);
        logger.debug("insert_es_info 同步es插入操作成功！database=" + database + ",table=" + table + ",json=" + json);
    }
}
